...
void find(char *path, char *name, char *origin) {
    char buf[512], *p;
    int fd;
    struct dirent de;
    struct stat st;
    ...
    switch (st.type) {
        ...
        *p++ = '/';
        ...
        strcpy(origin_path, origin);
        char *p2 = origin_path + strlen(origin_path);
        ...
        while (read(fd, &de, sizeof(de)) == sizeof(de)) {
            ...
            if (st.type == 1)  
            {   
                strcpy(origin, origin_path);
                find(de.name, name, origin);  
            } 
        }
        break;
  }
